...
async function fetchTrendingNewsFromGemini(): Promise<GeminiNewsItem[]> {
...
Ensure news is India-focused, relevant to current trends, and written in a journalistic tone.`;
  ...
  const parsed: GeminiNewsItem[] = JSON.parse(cleaned);
...
async function uploadNews() {
  ...
  try {
    ...
    const createdNews = await News.insertMany(
      newsItems.map((item) => ({
        title: item.title,
    ...
    console.log(
      `Successfully posted ${createdNews.length} trending news articles.`,
    );
  } catch (error) {
