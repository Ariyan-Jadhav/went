...
export const createComment = catchAsync(async (req, res) => {
    ...
    if (think) {
        await createNotification(think.user_id, userId, "comment", "commented on your think", interaction_id);
    }
    if (comment)
...
export const updateComment = catchAsync(async (req, res) => {
    ...
    try {
        const comment = await Comment.findByIdAndUpdate(comment_id, { content: content }, { new: true });
        if (comment) {
            await comment.save();
...
export const deleteComment = catchAsync(async (req, res) => {
    ...
    if (!comment_id)
        throw new AppError("comment_id is required", 400);
    if (!interaction_id)
        throw new AppError("interaction_id is required", 400);
