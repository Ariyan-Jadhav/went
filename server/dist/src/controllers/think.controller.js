...
export const createThink = catchAsync(async (req, res) => {
    ...
    if (!content)
        throw new AppError("content not found", 400);
    const parsedHashtags = Array.isArray(hashtags)
        ? hashtags
        : hashtags
            ? [hashtags]
            : [];
    const fixedHashtags = parsedHashtags.map((tag) => tag.startsWith("#") ? tag : `#${tag}`);
    ...
    if (images?.length > 0) {
        ...
        const uploadResults = await Promise.all(images.map((file) => uploadmedia(file.path, "went/thinks")));
        imageUrl = uploadResults.map((url) => ({
            url: url.secure_url,
            publicId: url.public_id,
        }));
        await Promise.all(images.map((file) => fs.unlink(file.path).catch(console.error)));
    }
...
export const updateThink = catchAsync(async (req, res) => {
    const { isAuthenticated, userId } = getAuth(req);
    if (!isAuthenticated)
        throw new AppError("User not authenticated", 401);
    if (!userId)
        throw new AppError("User not found", 401);
    const { content, hashtags, think_id } = req.body;
    const thinkForUpdate = await Think.findById(think_id);
    if (!thinkForUpdate)
        throw new AppError("Think not found", 404);
    if (thinkForUpdate.user_id !== userId) {
        throw new AppError("You can only edit your own thinks", 403);
    }
    const fixedHashtags = hashtags?.map((tag) => tag.startsWith("#") ? tag : `#${tag}`);
    const availableUpdates = {};
    if (content !== undefined)
        availableUpdates.content = content;
    if (hashtags !== undefined)
        availableUpdates.hashtags = fixedHashtags;
    ...
    if (availableUpdates) {
        ...
        if (!think)
            throw new AppError("Could not change the think", 400);
        await think.save();
...
export const deleteThink = catchAsync(async (req, res) => {
    const { isAuthenticated, userId } = getAuth(req);
    if (!isAuthenticated)
        throw new AppError("User not authenticated", 401);
    if (!userId)
    ...
    if (think_id) {
        try {
            ...
            const publicIds = think.imageUrl?.map((publicId) => deleteMediafromCloudinary(publicId.publicId));
            if (publicIds)
                await Promise.all(publicIds);
...
export const reThink = catchAsync(async (req, res) => {
    const { isAuthenticated, userId } = getAuth(req);
    if (!isAuthenticated)
        throw new AppError("User not authenticated", 401);
    if (!userId)
...
export const getThinksByUser = catchAsync(async (req, res) => {
    const { isAuthenticated, userId } = getAuth(req);
    if (!isAuthenticated)
        throw new AppError("User not authenticated", 401);
    if (!userId)
...
export const getUserRepostThink = catchAsync(async (req, res) => {
    const { isAuthenticated, userId } = getAuth(req);
    if (!isAuthenticated)
        throw new AppError("User not authenticated", 401);
    if (!userId)
...
//# sourceMappingURL=think.controller.js.map