...
async function fetchNewsFromNewsdata() {
    await connectDB();
    const response = await fetch(`https://newsdata.io/api/1/latest?apikey=${CONFIG.NEWSDATA_API_KEY}&country=in&language=en`);
    ...
    const article = data.results.find((a) => !a.duplicate && !postedIds.has(a.article_id));
    if (!article)
        throw new AppError("No new articles found", 500);
    ...
    if (!think)
        throw new AppError("could not post", 500);
...
//# sourceMappingURL=news2.0.js.map