...
async function fetchNewsFromMediastack() {
    await connectDB();
    const response = await fetch(`https://api.mediastack.com/v1/news?access_key=${CONFIG.NORTH_API_KEY}&countries=in&limit=5`);
    ...
    const article = data.data.find((a) => !postedUrls.has(a.url));
    if (!article)
