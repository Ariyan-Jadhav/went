...
async function getArtistImage(name, token) {
    try {
...
async function getAlbumImage(name, token) {
    try {
        const res = await fetch(`https://api.spotify.com/v1/search?q=${encodeURIComponent(name)}&type=album&limit=1`, { headers: { Authorization: `Bearer ${token}` } });
        if (res.status === 429) {
...
async function getTrackImage(name, token) {
    try {
        const res = await fetch(`https://api.spotify.com/v1/search?q=${encodeURIComponent(name)}&type=track&limit=1`, { headers: { Authorization: `Bearer ${token}` } });
        if (res.status === 429) {
...
async function getMovie(name) {
    try {
        const res = await fetch(`https://www.omdbapi.com/?apikey=${MOVIE_API_KEY}&t=${encodeURIComponent(name)}`);
        const data = await res.json();
...
async function seedBots() {
    ...
    console.log(`Seeding bots ${START + 1} to ${END} (${TEST_BOTS.length} bots)...`);
    ...
    for (const bot of validBots.filter((b) => b.favorites?.music?.favorite_track)) {
        console.log(`Track: ${bot.favorites.music.favorite_track}`);
        const image = await getTrackImage(bot.favorites.music.favorite_track, spotifyToken);
    ...
    for (const bot of validBots.filter((b) => b.favorites?.music?.favorite_album)) {
        console.log(`Album: ${bot.favorites.music.favorite_album}`);
        const image = await getAlbumImage(bot.favorites.music.favorite_album, spotifyToken);
    ...
    for (const bot of validBots.filter((b) => b.favorites?.music?.favorite_artist)) {
        console.log(`Artist: ${bot.favorites.music.favorite_artist}`);
        const image = await getArtistImage(bot.favorites.music.favorite_artist, spotifyToken);
