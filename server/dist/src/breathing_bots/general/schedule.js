...
async function uploadThink(data, user) {
...
Output only the post. No explanation.`;
    ...
    const chatCompletion = await groq.chat.completions.create({
        messages: [
            {
                role: "system",
                content: "You are a real human. Write casually, imperfectly, never like AI.",
            },
            {
                role: "user",
                content: prompt,
    ...
    output = output.trim();
    await BreathingBots.updateOne({ id: user }, { $push: { recent_outputs: output } });
...
async function processBot(bots, reload, setter) {
    if (bots.length === 0) {
    ...
    if (!data)
        throw new AppError("could not find the bot", 401);
